...
Locator: getByAltText('orangehrm-logo')
Expected: visible
Error: strict mode violation: getByAltText('orangehrm-logo') resolved to 2 elements:
...
Call log:
  ...
  - waiting for getByAltText('orangehrm-logo')
...
> 6 | await expect(page.getByAltText("orangehrm-logo")).toBeVisible();
    |                                                   ^ Error: expect(locator).toBeVisible() failed
  7 | 
  8 | 
  9 | })
